    ...
    >
      ...
      <template #footer>
        <template v-if="props.title==='recommend'">
          <van-button size="small" @click="enterTeam(team.id)">加入队伍</van-button>
        </template>
        <template v-if="props.title==='joinTeam'">
          <van-button size="small" >查看队伍</van-button>
          <van-button size="small" @click="doQuitTeam(team.id)">退出队伍</van-button>
        </template>
        <template v-if="props.title==='createTeam'">
          <van-button size="small" >查看队伍</van-button>
          <van-button size="small"  >更新队伍</van-button>
          <van-button size="small" @click="doDeleteTeam(team.id)" >解散队伍</van-button>
        </template>
      </template>
    </van-card>
  </div>
...
const doQuitTeam = async (id:number)=>{
  ...
  if (res?.code===0){
    ...
    showFailToast("退出队伍成功"+(res.description!=''?","+res.description:''))
  }
}
/**
 * 退出队伍
...
const doDeleteTeam = async (id:number)=>{
  console.log(route.query.password)
  const res = await myAxios.post("/team/delete",{
    id
  })
  if (res?.code===0){
    showSuccessToast("删除队伍失败！")
  }else {
